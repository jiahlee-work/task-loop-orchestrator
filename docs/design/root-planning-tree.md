...
Status: design draft with initial run directory artifact storage and Gemini root-contract/task-tree output implemented; executor root-contract injection, reviewer contract/evidence prompts, and root review decisions are also implemented. This document defines the intended planning model for the real Gemini Planner, Codex Executor, and OpenAI Reviewer loop. The current storage layer writes `root-contract.json`, `task-tree.json`, `state.json`, and `summary.md`; Gemini Planner now requests `rootContract` and `taskTree` JSON and preserves those artifacts in run state. Codex Executor receives the approved root contract plus the assigned task, and OpenAI Reviewer receives diff/test evidence, acceptance criteria, and context guard context. Root uses reviewer verdicts to complete, reschedule, block for owner decisions, or fail a branch. This does not mean the full tree executor is implemented.
...
8. `status`가 최신 root decision, owner decision, blocked/failed subtask를 보여 주게 한다.
9. `history`로 여러 run의 진행 상태와 root decision 요약을 읽기 전용으로 조회한다.
10. `report`로 run별 로컬 markdown handoff 문서를 생성한다.
