...
- `write-runner --intent <intentId> --preflight <path> --execute-local --json`
...
`write-runner --intent <intentId> --json`, `write-runner --intent <intentId> --preflight <path> --json`, `write-runner --intent <intentId> --preflight <path> --simulate --json`, and `write-runner --intent <intentId> --preflight <path> --execute-local --json` have a command-specific schema branch for the audited write runner dry-run, simulation, and guarded local verification boundary. The command computes the same readiness gate as `write-readiness`; when readiness is `ready`, it saves local dry-run trace records under `.orchestrator/execution-traces/` and returns a plan report. When readiness is `blocked` or `unknown`, it returns a blocked dry-run report without saving new traces. `--execute --json` is accepted only to return a disabled report; it does not run actual write execution.
...
The `writeRunnerExecutionPolicy` fixes `mode`, `requiredReadiness`, `allowedActions`, `disallowedActions`, `allowedVerificationActions`, `blockers`, `localExecutionEnabled`, `localExecutionScope`, `actualExecutionEnabled`, `executionEnabled`, and `writeExecution`. `mode` is `dry_run`, `simulate`, `execute_disabled`, or `execute_local`; `requiredReadiness` is `ready`; `localExecutionScope` is `verification_only` only for guarded local verification; and `actualExecutionEnabled` is always `false`. Each `writeRunnerSimulationResult` fixes `action`, `status`, `summary`, `executionEnabled`, `writeExecution`, and `hasExecutionResults` without raw command arguments or process output.

Each `writeRunnerLocalExecutionResult` fixes `action`, `status`, `summary`, `durationMs`, `outputCaptured`, `executionEnabled`, `writeExecution`, and `hasExecutionResults`. `action` is an allowlisted verification script id such as `typecheck`, `test`, `build`, `lint`, `package:smoke`, or `release:check`; `status` is `succeeded`, `failed`, `timed_out`, or `blocked`. The result intentionally omits raw command args, stdout, stderr, exit codes, stacks, secrets, and `executedCommands`.
...
The JSON path handles missing `--intent`, missing execution intents, invalid persisted execution intent files, invalid persisted execution trace files, missing preflight paths, file-not-found/read failures, invalid JSON, invalid preflight schema, missing verification actions, and non-allowlisted verification actions with safe error envelopes. This dry-run, simulation, and guarded local verification boundary does not create branches, commits, pushes, pull requests, merges, releases, or tags, and it does not expose raw command args, stdout, stderr, exit codes, `executedCommands`, stack traces, or secrets.
