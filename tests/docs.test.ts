...
describe("release readiness documentation", () => {
  ...
  it("keeps the write execution model as a disabled design draft", async () => {
    ...
    expectContainsAll(design, [
      ...
      "Status: plain and JSON paths enabled for `write-readiness --intent <intentId> [--json]`; JSON preflight input is enabled with `--preflight <path> --json`. Plain preflight output remains deferred.",
      ...
      "`--preflight <path>`: optional in JSON mode only",
      ...
      "`--preflight <path> --json` reads evidence only and must not echo raw file paths or raw file contents",
      "no file writes",
      ...
      "Plain `--preflight` output remains deferred and returns a short safe unsupported-preflight error",
      ...
      "optional plain `--preflight <path>` support only after human-readable error handling, docs updates, package smoke, no raw output echoing, and no-write policy are ready",
      ...
      "Enable the read-only `--preflight <path> --json` CLI option after CLI error handling and package smoke are covered",
      "npm publish",
      "git tag creation",
      "GitHub release creation",
...
describe("command reference documentation", () => {
  ...
  it("keeps per-command write-side boundaries explicit", async () => {
    ...
    expectSectionContains(sections, "write-readiness", [
      ...
      ".orchestrator/execution-intents/",
      ".orchestrator/execution-traces/",
      "JSON mode can also read a safe preflight evidence file with `--preflight <path>`",
      "loader/parser failures return JSON error envelopes with disabled execution markers instead of partial success",
      "Plain `--preflight` output is still deferred",
