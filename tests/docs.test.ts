...
describe("command reference documentation", () => {
  it("documents every implemented CLI command", async () => {
    ...
    expect(usageSignatures).toEqual([
      ...
      "run ISSUE-KEY [--note text] [--planner mock|gemini] [--permission read|write|maintainer] [--executor mock|codex-cli-dry-run|codex-cli] [--reviewer mock|local-evidence] [--max-iterations n] [--json]",
      "run <instruction> [--description text] [--planner mock|gemini] [--permission read|write|maintainer] [--executor mock|codex-cli-dry-run|codex-cli] [--reviewer mock|local-evidence] [--max-iterations n] [--json]",
  ...
  it("keeps command examples aligned with CLI and smoke flows", async () => {
    ...
    expectContainsAll(smokeScript, [
      ...
      "store_path",
      '"tlo", "init"',
      '"run", "Smoke task", "--planner", "mock", "--max-iterations", "1", "--json"',
