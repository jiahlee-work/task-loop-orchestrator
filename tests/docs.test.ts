...
describe("release readiness documentation", () => {
  ...
  it("keeps the write execution model as a disabled design draft", async () => {
    ...
    expectContainsAll(design, [
      ...
      "`source`: `preflight`",
      "map recognized preflight evidence into the existing `WriteExecutionReadinessPreflightInput` booleans",
      "Missing or unrecognized checks should remain `unknown` rather than implying approval",
      ...
      "Future preflight file parsing should stay outside the helper",
      ...
      "optional read-only `--preflight <path>` support",
