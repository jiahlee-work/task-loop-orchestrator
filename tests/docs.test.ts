...
describe("release readiness documentation", () => {
  ...
  it("keeps the write execution model as a disabled design draft", async () => {
    ...
    expectContainsAll(design, [
      ...
      "valid preflight plain success path",
      ...
      "optional read-only `--preflight <path>` support only after CLI error handling, docs/schema updates, package smoke, no raw output echoing, and no-write policy are ready",
      ...
      "Add the read-only `--preflight <path>` CLI option only after CLI error handling and package smoke are covered",
      "npm publish",
      "git tag creation",
      "GitHub release creation",
...
describe("command reference documentation", () => {
  ...
  it("keeps per-command write-side boundaries explicit", async () => {
    ...
    expectSectionContains(sections, "write-readiness", [
      ...
      "future `--preflight <path>` evidence input",
      "only designed in [design/write-execution-model.md](design/write-execution-model.md)",
      "safe error behavior",
      "are not accepted by the current CLI",
