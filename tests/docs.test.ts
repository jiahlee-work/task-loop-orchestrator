...
describe("quickstart documentation", () => {
  ...
  it("documents the first project command flow and GitHub checks fallback", async () => {
    ...
    expectContainsAll(quickstart, [
      ...
      "allowlisted package-script verification",
...
describe("release readiness documentation", () => {
  ...
  it("keeps the write execution model as a disabled design draft", async () => {
    ...
    expectContainsAll(design, [
      ...
      "`write-runner --intent <intentId> [--preflight <path>] [--simulate|--execute|--execute-local] [--verification <action>] --json` is enabled as an audited dry-run, execution-policy, simulated-executor, and guarded local verification boundary",
      ...
      "Keep `write-runner --intent <intentId> [--preflight <path>] [--simulate|--execute|--execute-local] [--verification <action>] --json` under schema/docs/package smoke coverage as a dry-run, simulation, and guarded verification boundary",
      ...
      "If readiness is `ready`, dry-run, simulate, and guarded local verification modes persist local dry-run trace records under `.orchestrator/execution-traces/` as audit artifacts",
      "If readiness is `blocked` or `unknown`, the CLI returns a blocked dry-run report and does not save new traces or run verification",
      "`localTracePersistence`",
      "`mode: \"dry_run\" | \"simulate\" | \"execute_disabled\" | \"execute_local\"`",
      "`actualExecutionEnabled: false`",
      "`--simulate` uses a deterministic safe executor boundary",
      "`--execute-local` is explicit opt-in for allowlisted package-script verification actions",
      "Guarded local verification uses Node process spawning with shell execution disabled",
      "timeout, and bounded output capture",
      "sanitized environment",
      "localExecutionResultCount",
      ...
      "They must not include raw command args, raw stdout, raw stderr, exit codes, stacks, secrets, `executedCommands`, or full package script output",
      "they do not expose raw command args",
      "This boundary is not a write-side execution engine",
      "must not use shell execution, GitHub write APIs, git branch creation",
      "branch/commit/push/PR write execution unlock as a separate milestone",
      ...
      "Add a guarded local verification executor alpha for allowlisted package scripts only",
      "npm publish",
      "git tag creation",
      "GitHub release creation",
...
describe("command reference documentation", () => {
  it("documents every implemented CLI command", async () => {
    ...
    expect(usageSignatures).toEqual([
      ...
      "write-runner --intent intentId [--preflight path] [--simulate|--execute|--execute-local] [--verification typecheck|test|build|lint|package:smoke|release:check] --json",
      "checks [ref] [--json]"
    ]);
  ...
  it("keeps per-command write-side boundaries explicit", async () => {
    ...
    expectSectionContains(sections, "write-runner", [
      ...
      "`--simulate` uses a deterministic safe executor boundary",
      "`--execute-local` is an explicit alpha opt-in",
      "allowlisted package-script verification actions",
