...
describe("release readiness documentation", () => {
  ...
  it("keeps the write execution model as a disabled design draft", async () => {
    ...
    expectContainsAll(design, [
      ...
      "avoid echoing the raw preflight file path or raw file contents",
      ...
      "Future `--preflight <path>` support must read evidence only and must not echo raw file paths or raw file contents",
      "no file writes",
      ...
      "optional read-only `--preflight <path>` support only after file reading, CLI error handling, no raw output echoing, and no-write policy are ready",
      ...
      "Add the read-only `--preflight <path>` CLI option only after file reading and safe error handling are covered",
      "npm publish",
      "git tag creation",
      "GitHub release creation",
...
describe("command reference documentation", () => {
  ...
  it("keeps per-command write-side boundaries explicit", async () => {
    ...
    expectSectionContains(sections, "write-readiness", [
      ...
      "future `--preflight <path>` evidence input is only designed",
      "is not accepted by the current CLI",
